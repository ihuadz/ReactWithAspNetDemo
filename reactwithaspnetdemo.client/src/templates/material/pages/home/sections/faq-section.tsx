...
import SectionTitle from '@/templates/material/components/section-title';
...
export default function FaqSection() {
  ...
  return (
    <section className='flex flex-col items-center justify-center mt-40'>
      <SectionTitle
        title="FAQ's"
        subtitle="Looking for answers to your frequently asked questions? Check out our FAQ's section below to find."
      />
        ...
        {data.map((item, index) => (
          ...
          >
            ...
            >
              {item.question}
              {isOpen === index ? (
                <Icon icon='lucide:minus' className='size-5 text-gray-500' />
              ) : (
                <Icon icon='lucide:plus' className='size-5 text-gray-500' />
              )}
            </h3>
            <p
